...
string _ltrim(const std::string& s)  //_Itrim returns a new string that is a substring of the original string s, with
									 //any leading whitespace characters removed.
{
	size_t start = s.find_first_not_of(WHITESPACE);
	return (start == std::string::npos) ? "" : s.substr(start);
}

string _rtrim(const std::string& s)  //_rtrim returns a new string that is a substring of the original string s, with
									 //any trailing whitespace characters removed.
{
	size_t end = s.find_last_not_of(WHITESPACE);
	return (end == std::string::npos) ? "" : s.substr(0, end + 1);
}

string _trim(const std::string& s)  //_trim returns a new string that is a substring of the original string s, with any
									//leading or trailing whitespace characters removed.
{
	return _rtrim(_ltrim(s));
}

int _parseCommandLine(
	const char* cmd_line,
	char** args)  //_parseCommandLine parses the command line string cmd_line into individual arguments and stores them
				  //in the array of strings args. The function returns the number of arguments parsed.
{
	FUNC_ENTRY()
		int i = 0;
...
bool _isBackgroundComamnd(
	const char* cmd_line)  // isBackgroundComamnd checks whether the input command line string ends with the & symbol to
						   // determine whether the command should be run in the background or not.
{
	const string str(cmd_line);
	return str[str.find_last_not_of(WHITESPACE)] == '&';
}

void _removeBackgroundSign(
	char* cmd_line)  //_removeBackgroundSign removes the background sign & from the end of the command line if it is
					 //present, and removes any trailing whitespace characters after the background sign.
{
	const string str(cmd_line);
...
void ChpromptCommand::execute() {
	...
	// throughout the program. In the case of the SmallShell class, calling SmallShell::getInstance() returns the same
	// instance every time it is called, so all code that interacts with the SmallShell class is working with the same
	// instance. This ensures that the state of the SmallShell instance is consistent throughout the program and avoids
	// problems that might arise from multiple instances of the class being created and potentially conflicting with each
	// other.
}

SmallShell::SmallShell()
	: promptName("smash>"), previousDR("empty"), currentJobPid(-1), currentJobId(-1), currentJobCommand("") {
	smash_pid = getpid();
	if (smash_pid == -1) {
...
void ShowPidCommand::execute() { std::cout << "smash pid is " << SmallShell::getInstance().getPidSmash() << std::endl; }
...
ChangeDirCommand::ChangeDirCommand(const char* cmd_line, vector<std::string>& args, int argSize)
	: BuiltInCommand(cmd_line), args(args), argsSize(argSize) {}
...
void JobsCommand::execute() { SmallShell::getInstance().jobs_list->printJobsList(); }
...
KillCommand::KillCommand(const char* cmd_line, vector<string>& args, int argsSize)
	: BuiltInCommand(cmd_line), args(args), argsSize(argsSize) {}
 ...
 */
ForegroundCommand::ForegroundCommand(const char* cmd_line, vector<std::string>& args, int argsSize)
	: BuiltInCommand(cmd_line), args(args), argsSize(argsSize) {}
...
void ForegroundCommand::execute() {
	...
	if (checkPid == myPid) {
		for (auto i = SmallShell::getInstance().jobs_list->jobs_entry_vec.begin();
			i < SmallShell::getInstance().jobs_list->jobs_entry_vec.end(); i++)
			if (i->jobPid == myPid) {
...
QuitCommand::QuitCommand(const char* cmd_line, vector<string>& args, int argsSize)
	: BuiltInCommand(cmd_line), args(args), argsSize(argsSize) {}
...
BackgroundCommand::BackgroundCommand(const char* cmd_line, vector<std::string>& args, int argsSize)
	: BuiltInCommand(cmd_line), args(args), argsSize(argsSize) {}
...
RedirectionCommand::RedirectionCommand(const char* cmd_line) : Command(cmd_line) {
	...
	size_t position_of_red_sign = str.find(">>");  // 'find' searches for '>' in 'str' and returns its position. if '>' is
												   // not found, 'find' returns 'string::npos'
...
void RedirectionCommand::execute() {
	...
	if (dup2(standard_out_channel, 1) ==
		-1) {  // restore the original stdout (replace the current stdout (the file) with the original stdout
		perror("smash error: dup2 failed");  //
		return;
	}
...
SetcoreCommand::SetcoreCommand(const char* cmd_line, vector<string>& args, int argsSize)
	: BuiltInCommand(cmd_line), args(args), argsSize(argsSize) {}
...
GetFileTypeCommand::GetFileTypeCommand(const char* cmd_line, vector<string>& args, int argsSize)
	: BuiltInCommand(cmd_line), args(args), argsSize(argsSize) {}
...
void GetFileTypeCommand::execute() {
	...
	cout << args[1] << "'s"
		<< " type is \"" << fileType << "\" and takes up " << size << " bytes" << endl;
}
...
Command* SmallShell::CreateCommand(const char* cmd_line) {
	...
	// split the cmd_line argument into individual words and store them in the myCMD array. The number of words is
	// returned and stored in argsSize.
...
void JobsList::removeFinishedJobs() {
	...
	/*for (auto it = jobs_entry_vec.begin(); it < jobs_entry_vec.end();++it) {
	  ...
	  // Check if the process has finished
		if (result == it->jobPid && !WIFCONTINUED(status) && !WIFSTOPPED(status)) { // If the process has finished or
	killed it = jobs_entry_vec.erase(it); // Remove the job from the vector
